...
function PlayState:enter(def)
    --get map data and create map
    self.levelnum = def.levelnum
    self.levelwidth = def.levelnum * 15 + 15
    self.level = LevelMaker.generate(self.levelwidth, 10)
    self.tileMap = self.level.tileMap
    self.playerScore = def.playerScore

    -- get the first solid ground we have so that we won't spawn our player on an empty area
    start = 0
    ...
    self.player.score = self.playerScore
