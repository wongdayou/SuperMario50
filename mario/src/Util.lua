...
function GenerateSubQuads(atlas, setsX, setsY, sizeX, sizeY, startX, startY)
    -- setsX: number of sets on horizontal plane
    -- setsY: number of sets on vertical plane
    -- sizeX: width of each set
    -- sizeY: height of each set
    -- startX: starting X of our atlas
    -- startY: starting Y of our atlas

    local sheetCounter = 1
    local spritesheet = {}
    local sheetWidth = sizeX * setsX - sizeX + startX
    local sheetHeight = sizeY * setsY - sizeY + startY

    for y = startY, sheetHeight, sizeY do
        for x = startX, sheetWidth, sizeX do
            spritesheet[sheetCounter] = 
                love.graphics.newQuad(x, y, sizeX, sizeY, atlas:getDimensions())
            sheetCounter = sheetCounter + 1
